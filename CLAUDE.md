...
- identity/file info: `hash`, `path`, `filename`, `date_added` (first-scan timestamp, ms precision, set once), `date_downloaded` (file mtime at first scan, second precision, set once), `duration`, `filesize_mb`, `fps`, `resolution`, `bitrate`
...
**Scanning** (`go_backend/internal/scanner/`) — Go-native. Walks all configured collection folders (`walk.go`), filters by extension and ignore list, computes a stable SHA-256 content hash from three 64 KB chunks of each file (`hash.go`), probes video attributes via ffprobe (`attributes.go`), parses each filename against `scene_filename_formats` using the `string_parser` package (`filename.go`), and merges results into the DB without clobbering manually-edited fields (`merge.go`). At the start of a full scan, all videos are marked `is_linked=0` via a single SQL UPDATE (`SetAllUnlinked`); each scanned video is written back with `is_linked=1`. Videos no longer found on disk remain in the DB with `is_linked=0` so interaction history survives reconnected drives. Triggered from the dashboard (`POST /api/dashboard/run-scan`) or directly. After a scan completes, Go shells out to `py/cmd/generateTFIDF.py` to rebuild the TF-IDF model.
...
**Filtering/sorting/catalogue** (`go_backend/internal/query/`) — the Go-native, non-TF-IDF half of search: structural filtering by actor/studio/collection/tags/date ranges/favorites, configurable sort, and catalogue aggregation (grouping videos by actor/studio/collection/tag with counts and "newest video" info for browsing pages). Common sort fields (`date_added`, `date_downloaded`, `date_released`, `title`, `filename`, `path`, `duration`, `bitrate`, `resolution`) use direct `slices.SortFunc` on struct fields via `handleSortByDirectFields`; any unrecognised field falls back to a JSON marshal→map→sort→unmarshal round-trip.
