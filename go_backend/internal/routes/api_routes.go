package routes

import (
    "cmp"
    "fmt"
    "math"
    "math/rand"
    "slices"
    "strconv"
    "strings"
    "time"

    "github.com/labstack/echo/v4"

    "cpv_backend/internal/config"
    "cpv_backend/internal/db"
    "cpv_backend/internal/pyworker"
    "cpv_backend/internal/schemas"
)

func IncludeApiRoutes(e *echo.Group, db_path string, actorInfoDir string, store *config.ConfigStore, stateStore *config.AppStateStore) {

    e.GET("/get/video-data/:hash",              func(c echo.Context) error { return ECHO_get_video_data(c, db_path) })
    e.GET("/get/random-video-hash",             func(c echo.Context) error { return ECHO_get_random_hash(c, db_path, stateStore) })
    e.GET("/get/random-spotlight-video-hash",   func(c echo.Context) error { return ECHO_get_spotlight_hash(c, db_path, stateStore) })
    e.GET("/get/movie/:movie_title",            func(c echo.Context) error { return ECHO_get_movie(c, db_path, stateStore) })
    e.GET("/get/movie-series/:movie_series",    func(c echo.Context) error { return ECHO_get_movie_series(c, db_path, stateStore) })
    e.GET("/get/line/:line",                    func(c echo.Context) error { return ECHO_get_line(c, db_path, stateStore) })
    e.GET("/get/actor/:name",                   func(c echo.Context) error { return ECHO_get_actor(c, db_path, actorInfoDir) })
    e.GET("/get/actor-video-count/:name",       func(c echo.Context) error { return ECHO_get_actor_vid_count(c, db_path, stateStore) })
    e.GET("/get/studio-video-count/:name",      func(c echo.Context) error { return ECHO_studio_vid_count(c, db_path, stateStore) })
    e.GET("/get/curated-collections",           func(c echo.Context) error { return ECHO_get_curated_collections(c, store) })

}


// ECHO_get_video_data — single video by hash, not pool-filtered
func ECHO_get_video_data(c echo.Context, db_path string) error {
    hash := c.Param("hash")
    vd, err := db.ReadSerializedRowFromTable[schemas.VideoData](db_path, "videos", hash)
    if err != nil {
        return handleServerError(c, 500, "Unable to read from database", err)
    }
    return c.JSON(200, vd)
}


// ECHO_get_random_hash
func ECHO_get_random_hash(c echo.Context, db_path string, stateStore *config.AppStateStore) error {
    vids, err := getFilteredVideos(db_path, stateStore)
    if err != nil {
        return handleServerError(c, 500, "Unable to read table", err)
    }
    if len(vids) == 0 {
        return c.String(404, "No linked videos in db")
    }
    return c.JSON(200, map[string]string{"hash": vids[rand.Intn(len(vids))].Hash})
}


// ECHO_get_spotlight_hash — seeded daily spotlight, stable within a 6-hour window
func ECHO_get_spotlight_hash(c echo.Context, db_path string, stateStore *config.AppStateStore) error {
    vids, err := getFilteredVideos(db_path, stateStore)
    if err != nil {
        return handleServerError(c, 500, "Unable to read table", err)
    }
    if len(vids) == 0 {
        return c.String(404, "No linked videos in db")
    }

    tm := time.Now().Add(-6 * time.Hour)
    seed_str := fmt.Sprintf("%d%d", tm.Year(), tm.YearDay())
    seed, _ := strconv.Atoi(seed_str)
    r := rand.New(rand.NewSource(int64(seed)))
    hsh := fmt.Sprintf("%012x", r.Intn(int(math.Pow(16, 12))))
    fmt.Println("[SPOTLIGHT] Generated random hash: " + hsh)

    hsh_i, _ := strconv.ParseInt(hsh, 16, 64)
    var closest_i int64 = 0
    for _, vd := range vids {
        vhsh_i, _ := strconv.ParseInt(vd.Hash, 16, 64)
        if absDiff(hsh_i, vhsh_i) < absDiff(hsh_i, closest_i) {
            closest_i = vhsh_i
            fmt.Printf("[SPOTLIGHT] New closest_i: %012x  (diff = %d)\n", closest_i, absDiff(hsh_i, closest_i))
        }
    }
    return c.String(200, fmt.Sprintf("%012x", closest_i))
}


// ECHO_get_movie
func ECHO_get_movie(c echo.Context, db_path string, stateStore *config.AppStateStore) error {
    movie_title := strings.ToLower(c.Param("movie_title"))
    vids, err := getFilteredVideos(db_path, stateStore)
    if err != nil {
        return handleServerError(c, 500, "Unable to read table", err)
    }

    var filtered []schemas.VideoData
    for _, vd := range vids {
        if vd.MovieTitle != "" && strings.ToLower(vd.MovieTitle) == movie_title {
            filtered = append(filtered, vd)
        }
    }

    title_cmp := map[string]string{}
    for _, vd := range filtered {
        title_cmp[vd.Hash] = formatStringForIntComparability(vd.Title)
    }
    slices.SortFunc(filtered, func(a, b schemas.VideoData) int {
        if a.DateReleased != b.DateReleased {
            return cmp.Compare(a.DateReleased, b.DateReleased)
        }
        return cmp.Compare(title_cmp[a.Hash], title_cmp[b.Hash])
    })
    return c.JSON(200, filtered)
}


// ECHO_get_movie_series
func ECHO_get_movie_series(c echo.Context, db_path string, stateStore *config.AppStateStore) error {
    movie_series := strings.ToLower(c.Param("movie_series"))
    vids, err := getFilteredVideos(db_path, stateStore)
    if err != nil {
        return handleServerError(c, 500, "Unable to read table", err)
    }

    var filtered []schemas.VideoData
    for _, vd := range vids {
        if vd.MovieSeries != "" && strings.ToLower(vd.MovieSeries) == movie_series {
            filtered = append(filtered, vd)
        }
    }

    title_cmp := map[string]string{}
    for _, vd := range filtered {
        title_cmp[vd.Hash] = formatStringForIntComparability(vd.Title)
    }
    slices.SortFunc(filtered, func(a, b schemas.VideoData) int {
        if a.DateReleased != b.DateReleased {
            return cmp.Compare(a.DateReleased, b.DateReleased)
        }
        return cmp.Compare(title_cmp[a.Hash], title_cmp[b.Hash])
    })
    return c.JSON(200, filtered)
}


// ECHO_get_line
func ECHO_get_line(c echo.Context, db_path string, stateStore *config.AppStateStore) error {
    line := strings.ToLower(c.Param("line"))
    if line == "" {
        return c.String(400, "Line cannot be empty")
    }
    vids, err := getFilteredVideos(db_path, stateStore)
    if err != nil {
        return handleServerError(c, 500, "Unable to read table", err)
    }

    var filtered []schemas.VideoData
    for _, vd := range vids {
        if strings.ToLower(vd.Line) == line {
            filtered = append(filtered, vd)
        }
    }
    slices.SortFunc(filtered, func(a, b schemas.VideoData) int {
        if a.DateReleased != b.DateReleased {
            return cmp.Compare(a.DateReleased, b.DateReleased)
        }
        return cmp.Compare(a.DateAdded, b.DateAdded)
    })
    return c.JSON(200, filtered)
}


// ECHO_get_actor — actor info from Babepedia; not pool-filtered
func ECHO_get_actor(c echo.Context, db_path string, actorInfoDir string) error {
    name := c.Param("name")
    data, err := pyworker.ExecOutput[map[string]any](
        "-m", "cmd.getActorInfo",
        "--name", name,
        "--actor-info-dir", actorInfoDir,
    )
    if err != nil {
        return handleServerError(c, 500, "Python subprocess failed", err)
    }
    return c.JSON(200, data)
}


// ECHO_get_actor_vid_count
func ECHO_get_actor_vid_count(c echo.Context, db_path string, stateStore *config.AppStateStore) error {
    name := strings.ToLower(c.Param("name"))
    vids, err := getFilteredVideos(db_path, stateStore)
    if err != nil {
        return handleServerError(c, 500, "Unable to read table", err)
    }

    count := 0
    for _, vd := range vids {
        for _, act := range vd.Actors {
            if strings.ToLower(act) == name {
                count++
            }
        }
    }
    return c.JSON(200, map[string]int{"video_count": count})
}


// ECHO_studio_vid_count
func ECHO_studio_vid_count(c echo.Context, db_path string, stateStore *config.AppStateStore) error {
    name := strings.ToLower(c.Param("name"))
    vids, err := getFilteredVideos(db_path, stateStore)
    if err != nil {
        return handleServerError(c, 500, "Unable to read table", err)
    }

    count := 0
    for _, vd := range vids {
        if strings.ToLower(vd.Studio) == name || strings.ToLower(vd.Line) == name {
            count++
        }
    }
    return c.JSON(200, map[string]int{"video_count": count})
}


// ECHO_get_curated_collections — config data, not pool-filtered
func ECHO_get_curated_collections(c echo.Context, store *config.ConfigStore) error {
    cfg := store.Current()
    collections := cfg.CuratedCollections
    if collections == nil {
        collections = []config.CuratedCollection{}
    }
    return c.JSON(200, collections)
}
