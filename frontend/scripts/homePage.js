...
makeApiRequestGET('get-random-spotlight-video', [], (arg) => {
    
    console.log(arg);
    makeApiRequestGET('get-video', [arg.hash], (videodata) => {
        
        let date = Date().split(' ').slice(0,4).join(' ');
        document.querySelector('.top-container h2').innerText += ' ' + date;
        
        makeApiRequestGET('get-similar-videos', [videodata.hash, 0, related_videos_load_amount], search_results => {
        ...
        document.getElementById('expand-results-button').addEventListener('click', arg => {
            ...
            makeApiRequestGET('get-similar-videos', [videodata.hash, related_videos_loaded, related_videos_load_amount], search_results => {
                generate_results(search_results);
            });
            related_videos_loaded += related_videos_load_amount;
        });
