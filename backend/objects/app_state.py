...
from ..recommender import tfidf, similarity
...
class AppState:
    ...
    def load_tfidf(self, regen_tfidf_profiles: bool = False, recalculate_performer_embeddings: bool = False):
        ...
        self.tfidf_model = general.pickle_load(tfidf_model_fn)
        retrain_model = (self.tfidf_model is None or regen_tfidf_profiles)
        if self.tfidf_model and not regen_tfidf_profiles:
            print('[TFIFD] Loaded TF-IDF model')
            video_hashes = list(self.videos_dict.keys())
            if similarity.newHashNotInTFIDF(video_hashes, self.tfidf_model['hash_index_map']):
        ...
        if retrain_model:
            ...
            self.tfidf_model = tfidf.generate_tfidf_model(video_objects)
            print('[TFIFD] Done. Took {:.2f}s'.format(time.time()-start))
            print('[TFIFD] Saving TF-IDF model ...')
            general.pickle_save(self.tfidf_model, tfidf_model_fn)
