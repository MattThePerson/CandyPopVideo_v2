<script lang="ts">
    ...
    // Score a fuzzy match against target. Returns -1 if no match.
    ...
    let filtered = $derived((() => {
        ...
        const pool = items.filter(i => i !== value);
...
<div class="combobox">
    ...
    {#if value}
        <div class="chips">
            <span class="chip">
                {value}
                <button class="chip-x" onclick={() => { value = ''; }} aria-label="Clear selection">×</button>
            </span>
        </div>
    {/if}
</div>
